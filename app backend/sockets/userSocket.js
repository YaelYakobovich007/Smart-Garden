...
function handleUserSocket(ws) {
  ...
  ws.on('message', (msg) => {
    ...
    } else if (['UPDATE_PLANT_SCHEDULE', 'GET_IRRIGATION_RESULT', 'IRRIGATE_PLANT', 'STOP_IRRIGATION', 'OPEN_VALVE', 'CLOSE_VALVE', 'GET_VALVE_STATUS', 'UNBLOCK_VALVE', 'TEST_VALVE_BLOCK', 'RESTART_VALVE', 'CHECK_SENSOR_CONNECTION', 'CHECK_VALVE_MECHANISM', 'CHECK_POWER_SUPPLY'].includes(data.type)) {
